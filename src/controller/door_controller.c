...
void update_door() {
    ...
    int has_been_ordered_here =
        is_inside_ordered(get_current_floor()) ||
        is_outside_ordered(get_current_floor(), current_direction);
    ...
    int door_wait_time_over = door_opened_timestamp <
                              get_timestamp_in_milliseconds() - DOOR_WAIT_TIME;
